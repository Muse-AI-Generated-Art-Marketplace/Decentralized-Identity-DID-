...
const RATE_LIMIT_CONFIGS = {
  ...
  general: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_GENERAL_WINDOW_MS) || 15 * 60 * 1000, // 15 minutes
  ...
  contractRead: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_CONTRACT_READ_WINDOW_MS) || 5 * 60 * 1000, // 5 minutes
    ...
    message: {
      ...
      retryAfter:
        Math.ceil(
          (parseInt(process.env.RATE_LIMIT_CONTRACT_READ_WINDOW_MS) ||
            5 * 60 * 1000) / 60000,
        ) + " minutes",
    },
  },
  ...
  contractWrite: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_CONTRACT_WRITE_WINDOW_MS) ||
  ...
  deployContract: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_DEPLOY_CONTRACT_WINDOW_MS) ||
      60 * 60 * 1000, // 1 hour
  ...
  issueCredential: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_ISSUE_CREDENTIAL_WINDOW_MS) ||
      5 * 60 * 1000, // 5 minutes
  ...
  createAccount: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_CREATE_ACCOUNT_WINDOW_MS) ||
      60 * 60 * 1000, // 1 hour
  ...
  fundAccount: {
    windowMs:
      parseInt(process.env.RATE_LIMIT_FUND_ACCOUNT_WINDOW_MS) || 60 * 60 * 1000, // 1 hour
    ...
    message: {
      ...
      retryAfter:
        Math.ceil(
          (parseInt(process.env.RATE_LIMIT_FUND_ACCOUNT_WINDOW_MS) ||
            60 * 60 * 1000) / 3600000,
        ) + " hour",
    },
  },
...
const getRateLimitStatus = async () => {
  if (!redisClient) {
    return {
      status: "memory_store",
      message: "Using memory store for rate limiting",
    };
  }
